...
> **v2.2.1 已发布** — 新增 `-p` 模式假突跃自动识别与排除（测量时间间隔切换处的一阶导伪极值点），橙色空心圆圈标记 + 终端中文警告；文件保存重命名改为无限层级「是否替换」交互确认。v2.2.0 重大更新：将底层绘图引擎从 matplotlib 迁移至 plotly，**彻底解决了中文字体在各操作系统上的兼容性问题**。详见下方 [命令详解](#命令详解) 与 [GitHub Releases](https://github.com/EricZhangpku/kulun/releases)。
...
- 红色曲线：一阶导数 d*E*/d*t*
- 红色三角：突跃点（导数极值点），自动排除测量间隔切换引起的假突跃
